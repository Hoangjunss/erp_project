package com.project.erp.logging;
...
public class LoggingAspect  {
    ...
    @Before("execution(* com.project.erp.services.*.*(..))")
...
//    }
    ...
    @AfterReturning(pointcut = "execution(* com.project.erp.services.*.*(..))", returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        logger.info("After returning - Method: {} returned: {}", joinPoint.getSignature().getName(), result);
    }

    @AfterThrowing(pointcut = "execution(* com.project.erp.services.*.*(..))", throwing = "exception")
